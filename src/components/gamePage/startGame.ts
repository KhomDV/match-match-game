...
export function startGame(): void {
  ...
  if (userStyles === 'flex') {
    ...
    if (dbSettings.Cards?.trim() === 'Cars') {
      cat = two;
    } else if (dbSettings.Cards?.trim() === 'Animals') {
      cat = one;
    } else if (dbSettings.Cards?.trim() === 'Smile') {
      cat = three;
    } else {
      cat = one;
    }
    ...
    let images = cat.images.map((name) => `${cat.category}/${name}`);
    const diffMax = difficultyMax * difficultyMax;
    images = images.slice(0, diffMax / 2);
    newGame(images);
  } else {
